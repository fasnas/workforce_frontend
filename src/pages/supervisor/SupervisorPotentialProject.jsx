// SupervisorPotentialProject.jsx
import React, { useEffect, useState } from 'react';
import axiosInstance from '../../services/AxiosInstance';
import { useNavigate } from 'react-router-dom';
import {
  MapPin, User, ArrowRight, Plus,
  FolderOpen, Search, Calendar, Clock
} from 'lucide-react';
...
const SupervisorPotentialProject = () => {
  ...
  const getPendingProjects = async () => {
    try {
      const res = await axiosInstance.get('/getallpendigprojects');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Projects pending admin approval
          </p>
        </div>

        {/* Stats pill + Add button */}
        <div className="flex items-center gap-3 flex-shrink-0">
          {!loading && projects.length > 0 && (
            <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-2xl px-4 py-2.5">
              <Clock size={14} className="text-amber-500" />
              <span className="text-sm font-bold text-amber-700">{projects.length} pending</span>
            </div>
          )}
          <button
            onClick={() => navigate('/supervisor/addproject')}
            className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700
              active:scale-95 text-white text-sm font-bold rounded-2xl transition-all
              shadow-sm shadow-indigo-200 cursor-pointer whitespace-nowrap"
          >
            <Plus size={16} />
      ...
      {!loading && projects.length > 0 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 text-sm font-medium bg-white border border-gray-200
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm flex flex-col items-center justify-center py-20 px-6 text-center">
          ...
          <h2 className="text-base font-extrabold text-gray-800">
            {search ? 'No results found' : 'No Pending Projects'}
          </h2>
          <p className="text-sm text-gray-400 mt-1.5 max-w-xs">
            {search
              ? `No projects match "${search}". Try a different search.`
              : 'No potential projects yet. Create one to get started.'}
          </p>
          <div className="flex gap-3 mt-6">
            {search && (
              <button
                onClick={() => setSearch('')}
                className="px-4 py-2.5 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors cursor-pointer"
              >
                Clear Search
              </button>
            )}
            <button
              onClick={() => navigate('/supervisor/addproject')}
              className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-colors cursor-pointer"
            >
