...
/* ─── Main ────────────────────────────────────────────────────────────── */
const SupervisorSingleOngoingProject = () => {
  const { id } = useParams();
        ...
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">{timeLogs.length} {timeLogs.length === 1 ? 'entry' : 'entries'}</p>
            </div>
            ...
            {timeLogs.length === 0 ? (
              ...
              <div className="space-y-2">
                {[...timeLogs]
                  ...
                  .map((log, i) => <LogRow key={log._id} log={log} index={i} />)}
              </div>
            )}
          </div>
        </div>
...
export default SupervisorSingleOngoingProject;